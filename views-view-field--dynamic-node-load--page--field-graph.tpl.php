...
function drawShape(ev, id) {
  ...
  if (outPortsArray) {
    ...
    if(imagePath == 'default.png'){
      imgPath = 'http://localhost/wiki3/sites/default/files/default_images/default.png';
    }
    else{
...
function removeCreatedFiles(hiddenId){
  jQuery.ajax({
        url : 'http://localhost/wiki3/removing-created-files',
        data : {
          hiddenId : hiddenId
        },
        beforeSend : function() {
...
//Function for showing source read data
 function result_data(sourceJsonData) {
 	var sourceJsonData = read_json_file(sourceJsonData);
...
function read_json_file(jsonFile) {
        ...
        jQuery.ajax({
					  'async': false,
						'global': false,
						'url': "http://localhost/wiki3/sites/default/files/operator_files/"+jsonFile,
						'dataType': "text",
						'success': function(data) {
...
//Ajax request for sorting data.
 function applied_sort(sourceJsonData, colNames, attrValue, sortCellView, getSortCellById, hiddenId) {
  //console.log(sourceJsonData);
  jQuery.ajax({
        url : 'http://localhost/wiki3/sorting-data',
        data : {
          sourceJsonData : sourceJsonData,
          colNames: colNames,
          attrValue: attrValue,
...
//Ajax request for joins data.
 function applied_joins(sourceJsonData1, sourceJsonData2, joinType, getCellById, hiddenId) {
  //console.log(sourceJsonData);
  jQuery.ajax({
        url : 'http://localhost/wiki3/joins-data',
        data : {
...
jQuery("#submitFilters").on('click', function (){
  ...
  for (var i = 1; i <= globalfilersCount; i++) {
    ...
    if (i > 1) {
      ...
      colnamesArr.push(colname.replace(/\s+/g, '.'));
...
jQuery("#submitSorting").on('click', function (){
  ...
  var colNames = JSON.stringify(colNamesVal.replace(/\s+/g, '.'));
  console.log("-------------");
  console.log(colNames);
  console.log("-------------");
